...
class Pagination {
	...
	public function initialize($config = array())
	{
		...
		if ($this->query_string !== '')
		{
			...
			// Create full URL
			$base_url = ($this->base_url === '') ? SimpleMvc::$current_uri : $this->base_url;
			$this->url = SimpleMvc::$base_uri . $this->base_url.'?'.str_replace('%7Bpage%7D', '{page}', http_build_query($_GET));
      ...
      $this->url = explode('/', $this->base_url === '' ? SimpleMvc::$current_uri : $this->base_url);

			// Convert uri 'label' to corresponding integer if needed
			if (is_string($this->uri_argument))
			{
				if (($key = array_search($this->uri_argument, $this->url)) === FALSE)
				{
					// If uri 'label' is not found, auto add it to base_url
					$this->url[]= $this->uri_argument;
					$this->uri_argument = count($this->url) + 1;
				}
				else
				{
					$this->uri_argument = $key + 2;
				}
			}

			// Insert {page} placeholder
			$this->url[$this->uri_argument - 1] = '{page}';

			// Create full URL
			$this->url = implode('/', $this->url).SimpleMvc::$query_string;

			// Extract current page, -2 leaves out the controller and method
      if(isset(SimpleMvc::$arguments[$this->uri_argument-2])) {
			  $this->current_page = SimpleMvc::$arguments[$this->uri_argument-2];
      } else {
        $this->current_page = 1;
      }
		}
