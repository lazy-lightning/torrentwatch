<div id="feed-<?php echo $feed->id; ?>">
 <ul class='feedItems'>
  <?php $n=1;foreach($feedItems[$feed->id] as $feedItem): ?>
   <li class='torrent match_<?php echo reset(explode(' ',$feedItem->statusText)).(++$n%2?' alt':''); ?>' 
       title='<?php echo CHtml::encode($feedItem->description); ?>'>
