 ...
 * @since 1.1
 */
class NDbFixtureManager extends CApplicationComponent
{
	/**
  ...
  public function getFixtureFile($filename)
  {
	...
	{
    $initFile=$this->getFixtureFile($this->initScript);
	...
	{
    $initFile=$this->getFixtureFile($tableName . $this->initScriptSuffix);
		if(is_file($initFile))
			require($initFile);
		else
	...
	{
    $fileName=$this->getFixtureFile($tableName.'.php');
		if(!is_file($fileName))
			return false;
